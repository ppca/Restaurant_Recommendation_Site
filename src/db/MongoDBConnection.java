...
public class MongoDBConnection implements DBConnection {
	...
	public JSONObject getRestaurantsById(String businessId, boolean isVisited) {
		// same as using find(new Document());
		// use if you are sure there's only one object
		// eq: equals
		FindIterable<Document> iterable = db.getCollection("restaurants").find(eq("business_id", businessId));
		try {
			// only one object for one business_id
	...
	@Override
	// this is the same as MySQLConnection
	// DBConnection(interface)
	// AbstractDBConnection(abstract class)
	// we can let MySQL and MongoDB connection inherit the abstract class
